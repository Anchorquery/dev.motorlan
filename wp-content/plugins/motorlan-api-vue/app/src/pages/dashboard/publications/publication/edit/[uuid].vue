...
onMounted(async () => {
  try {
    const marcasRequest = useApi('/wp-json/motorlan/v1/marcas', { immediate: false }).get().json()
    const categoriesRequest = useApi('/wp-json/motorlan/v1/publicacion-categories', { immediate: false }).get().json()
    const tiposRequest = useApi('/wp-json/motorlan/v1/tipos', { immediate: false }).get().json()
    ...
    if (marcasRequest.data.value) {
      ...
      marcas.value = marcasRequest.data.value.map((marca: { term_id: any; name: any }) => ({
        title: marca.name,
        value: marca.term_id,
      }))
    }
    ...
    if (categoriesRequest.data.value) {
      categories.value = categoriesRequest.data.value.map((category: { name: any; term_id: any }) => ({
        name: category.name,
        id: category.term_id,
      }))
    }
    ...
    if (tiposRequest.data.value) {
      tipos.value = tiposRequest.data.value.map((tipo: { name: any; term_id: any; slug: any }) => ({
        title: tipo.name,
        value: tipo.term_id,
        slug: tipo.slug,
      }))
    ...
    if (motorUuid) {
      const motorRequest = useApi(`/wp-json/motorlan/v1/publicaciones/uuid/${motorUuid}`, { immediate: false }).get().json()
      await motorRequest.execute()

      if (motorRequest.error.value) {
        showToast(t('edit_publication.fetch_error', 'No se pudo cargar la publicacion.'), 'error')
        return
      }

      const post = motorRequest.data.value
      if (!post) {
        showToast(t('edit_publication.fetch_error', 'No se encontro la publicacion.'), 'error')
        return
      }
