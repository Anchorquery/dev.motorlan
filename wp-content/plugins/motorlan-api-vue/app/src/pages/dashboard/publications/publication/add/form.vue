...
// Auto-generate Title based on Nomenclature
const generatedTitle = computed(() => {
  // 1. Tipo
  let tipoName = ''
  if (postData.value.tipo.length && tipos.value.length) {
     const t = tipos.value.find(x => x.value === postData.value.tipo[0])
     if (t) tipoName = t.name
  }

  // 2. Marca
  let marcaName = ''
  if (postData.value.acf.marca && marcas.value.length) {
      const m = marcas.value.find(x => x.value === postData.value.acf.marca)
      if (m) marcaName = m.name
  }

  // 3. Modelo
  const modelo = postData.value.acf.tipo_o_referencia || ''

  // 4. Potencia o Par
  let powerOrTorque = ''
  if (postData.value.acf.potencia) {
      powerOrTorque = `${postData.value.acf.potencia} kW`
  } else if (postData.value.acf.par_nominal) {
      powerOrTorque = `${postData.value.acf.par_nominal} Nm`
  }

  // 5. Velocidad
  const velocidad = postData.value.acf.velocidad ? `${postData.value.acf.velocidad} rpm` : ''
...
// Update title when generatedTitle changes
watch(generatedTitle, (newVal) => {
  if (newVal) {
    postData.value.title = newVal
  }
})
...
const isMotor = computed(() => {
  if (postData.value.tipo.length && tipos.value.length) {
     const t = tipos.value.find(x => x.value === postData.value.tipo[0])
     // Check for 'motor' slug. Adjust if slug logic changes.
     return t && t.slug === 'motor'
  }
  return false
})
...
const ensureUserSession = async () => {
  // Si ya estamos logueados (síncrono por bootstrap o asíncrono ya resuelto)
  if (userStore.getIsLoggedIn && userStore.getUser?.id) {
    userData.value = userStore.getUser
    return true
  }

  // Si está cargando (fetch dramático inicial), esperamos
  if (userStore.isLoading) {
    let retries = 0
    while (userStore.isLoading && retries < 20) {
  ...
  // Si llegamos aquí, no está logueado o falló el fetch
  showToast(t('add_publication.toasts.login_required', 'Debes iniciar sesión para continuar.'), 'error')
  router.push('/login')
...
onMounted(async () => {
  try {
    const marcasRequest = useApi<Marca[]>('/wp-json/motorlan/v1/marcas', { immediate: false }).get().json()
    const categoriesRequest = useApi<Categoria[]>('/wp-json/motorlan/v1/publicacion-categories', { immediate: false }).get().json()
    const tiposRequest = useApi<Tipo[]>('/wp-json/motorlan/v1/tipos', { immediate: false }).get().json()

    await Promise.allSettled([
      marcasRequest.execute(),
      categoriesRequest.execute(),
      tiposRequest.execute(),
    ])

    if (marcasRequest.error.value)
      showToast(t('add_publication.toasts.fetch_brands_error', 'No se pudieron cargar las marcas.'), 'error')

    if (categoriesRequest.error.value)
      showToast(t('add_publication.toasts.fetch_categories_error', 'No se pudieron cargar las categorias.'), 'error')

    if (tiposRequest.error.value)
      showToast(t('add_publication.toasts.fetch_types_error', 'No se pudieron cargar los tipos.'), 'error')

    if (marcasRequest.data.value) {
      marcas.value = marcasRequest.data.value.map((marca: Marca) => ({
    ...
    if (categoriesRequest.data.value) {
      categories.value = categoriesRequest.data.value.map((category: Categoria) => ({
    ...
    if (tiposRequest.data.value) {
      tipos.value = tiposRequest.data.value.map((tipo: Tipo) => ({
              ...
              <VRow>
                ...
                >
                  <AppTextField
                    ...
                    :rules="[requiredValidator]"
                    hint="Este precio es referencial para tu control y cálculo de ofertas. No será visible para los compradores (siempre verán 'Consultar precio')."
                    persistent-hint
                  />
                </VCol>
                <VCol
